...
void print_matrix(const T* A, const int rowsA, const int colsA,
  ...
  for (int i = 0; i < rowsA; i++) {
    for (int j = 0; j < colsA; j++) {
      std::cout << std::setw(prec_save) << std::fixed << std::setprecision(prec_save) << A[j * rowsA + i] << ",\t";
    }
    std::cout << std::endl;
  }
  std::cout << std::endl;
...
/************************************/
template <class T>
void save_data(const T * A, const char *filename, const int matrixSize, const int numThreads) {
